...
- **COMBO（英雄组合）**：按同一战队、同一小局统计中野与 AD/辅助英雄组合，支持跨赛事汇总、最低选取次数过滤、选取率和组合胜率。
...
- 英雄组合的事实数据随英雄采集一起发布。升级到 V12 后需对既有赛段重新执行英雄采集，才能生成单局阵容；组合选取率的分母是该战队在所选范围内阵容完整的有效小局数，组合胜率为该组合获胜小局数除以组合选取次数。
